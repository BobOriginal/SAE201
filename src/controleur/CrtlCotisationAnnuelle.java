...
public class CrtlCotisationAnnuelle {
  ...
  void initialize() {
    ...
    BooleanBinding rien = Bindings.equal(listeCotisation.getSelectionModel().selectedIndexProperty(), -1);
    ...
    // griser le bouton COtisations 
    bnCotisations.setDisable(true);
  }
  @FXML
  void ouvrirRappel(ActionEvent event) throws IOException {
    Main.ouvrirRappel(event);
    Main.fermerCotisation(event);
  }

  @FXML
  void ouvrirFacturations(ActionEvent event) throws IOException {
    Main.ouvrirFacture(event);
    Main.fermerCotisation(event);
  }

  @FXML
  void ouvrirMain(ActionEvent event) throws IOException {
